...
- [x] 生徒別タブ：検索ボックス付きの生徒リスト（表示名・学籍番号で絞り込み、`/teacher/students` を参考にした UI）
  - 生徒をクリックすると個人詳細ページへ
  - ※当初はヒートマップ案だったが、フィルタ選択なしで生徒に直接たどり着ける形を優先してリスト形式に変更
- [x] `/teacher/students/[userId]` 個人詳細ページ（全体サマリー・単元別平均・レッスン別の最新得点率/受験回数/メモ件数）
  - 集計は生徒ダッシュボード（20a）の `buildStudentDashboard` を再利用
- [x] 生徒一覧の表示名から個人ページへリンク
...
> 実装・開発環境での動作確認とも完了し、main にマージ済み。残課題は Phase 22 を参照。
...
## Phase 22: コーディングプレイグラウンドの残課題対応

> Phase 21（コーディングプレイグラウンド）を実運用前にレビューした結果、優先度の高い3件の課題が見つかった。以下の順で対応する（ユーザー承認済みの優先順位。指示なく順番を変えない）。
> なお「実行結果は最新1回分のみで履歴がない」「標準ライブラリ・print/console.logベースに限定」の2点は、対応不要と判断済み（前者はメモ保存機能で代替可能、後者はこの範囲が情報Ⅰ基礎教育としての意図した仕様であり、これを超える用途は別のコーディングスペースを使うべきという方針のため）。
