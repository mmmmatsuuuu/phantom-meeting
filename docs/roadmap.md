...
### 設問別正答率
| # | 問題文（要約） | 形式 | 全体 | A組 | B組 | C組 | D組 |
|---|---|---|---|---|---|---|---|
| Q1 | 〇〇とは何か | 選択式 | 72% | 68% | 78% | 70% | 71% |
| Q2 | 手順を正しく並べよ | 並び替え | 44% | 40% | 50% | 42% | 44% |
| Q3 | 〇〇を説明せよ | 記述式 | — | — | — | — |

### Q2 誤答の内訳（正答率 44% ≤ 60% のため表示）
| 選択肢 | 人数 | 割合 |
|---|---|---|
| 「A→B→C」（正解） | 34人 | 44% |
| 「B→A→C」 | 28人 | 36% |
...
- [ ] `GET /api/teacher/units/[unitId]/quiz-export` を実装（teacher/admin のみ）
  - Markdown 形式でレスポンス（`Content-Disposition: attachment` でダウンロード）
  - 正答率 60% 以下の選択式・並び替え設問のみ誤答内訳を含める
  - 記述式はランダム3件のテキストを含める（学籍番号なし）
- [ ] `/teacher/contents` 単元行にエクスポートボタンを追加
  - クリックでそのままダウンロード（モーダル不要、学年全体固定）
  - 小テストが存在しない単元はボタンを非表示
  - ダウンロード後に表示するAI分析プロンプトテンプレートをコピーできるツールチップ or トーストを設置
    - 「【単元計画】\n（ここに単元計画を貼る）\n\n【小テスト結果】\n（ダウンロードしたMarkdownを貼る）\n\n---\n以上をもとに、次の単元以降で気を付けるべき授業改善案を提案してください。」
