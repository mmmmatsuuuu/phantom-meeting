...
## Phase 9: エディタ強化 + データ出力（生徒向け）

> メモ体験の向上と、生徒自身がデータを持ち出せる手段を提供する。
...
- [ ] 教師用 CSV 出力
  ...
  - Phase 8 完了後は小テスト回答データも対象に追加
- [ ] 出力 UI の配置先を検討（`/teacher/contents` 内か、専用ページか）

### 備考

> Phase 10 の実装時点で、教師・生徒それぞれの**ダッシュボードページ**の必要性を再検討する。
...
> 授業運用が安定してから着手する。何をどこに追加するかを先に具体化する。
