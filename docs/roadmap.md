...
- [ ] 単元の小テスト結果を Markdown / CSV 形式でエクスポートする機能
  - 設問別正答率・生徒別回答（学籍番号）・記述回答テキストを含む構造化フォーマット
  - 学年・クラス絞り込み対応（`student_number` 桁構造を利用、Phase 13 / 15.9 と共通）
  - `/teacher/contents` 単元行にエクスポートボタンを追加（モーダルで学年・クラス・形式を選択）
  - AIに渡す分析プロンプトのテンプレートをモーダル内に提供（コピー可）
    - 「つまずきポイントと授業改善案を教えてください」
    - 「次回の教材に追加すべき補足説明を提案してください」
