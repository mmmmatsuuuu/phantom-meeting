...
## Phase 15.95: 小テスト結果エクスポート＋AI分析連携（教師向け）

> 単元終了後の小テスト結果を生成AIに読み込ませやすい形式でエクスポートし、教材改善や生徒へのフィードバック作成に活用できる仕組みを提供する。

### 背景・目的

- 単元が終了し生徒の小テスト結果が揃った段階で「つまずきポイントの特定」「個別フィードバック」を効率よく行いたい
- Phase 15.9 のヒートマップで「どの設問が難しかったか」は把握できるが、「なぜ間違えたか」「どのような答えを書いたか」まで踏み込んだ分析には生成AIとの連携が有効
- 教師がエクスポートしたデータを Claude 等に貼り付けて分析を依頼し、その結果を次回授業の資料修正や生徒へのコメントに活かす

### 実装フロー

```
単元の小テスト結果をエクスポート（Markdown / CSV）
  ↓
（外部）生成AIにデータ＋プロンプトを貼り付けて分析を依頼
  ↓
分析結果をもとに授業改善・生徒フィードバックを作成
  ↓
（任意）フィードバック内容を生徒一覧の備考欄（/teacher/students）に記入
```
...
| # | 問題文 | 形式 | 正答率 |
|---|---|---|---|
| Q1 | ○○とは何か？ | 選択式 | 72%（13/18人） |
| Q2 | ○○の手順を正しい順に並べよ | 並び替え | 44%（8/18人） |
| Q3 | ○○について説明せよ | 記述式 | （自己採点） |
...
- 行 = 生徒（学籍番号）、列 = 設問（Q1, Q2, …）
- 選択式・並び替え：`○` / `×` / `未提出`
- 記述式：回答テキスト

### AI分析用プロンプトテンプレート

エクスポートモーダル内に以下のテンプレートをコピーできる形で提供する：

**つまずきポイント分析（授業改善向け）**
```
以下は○○単元の小テスト結果データです。
各設問で生徒がどのような誤解・つまずきをしているか分析し、
次回の授業で補足すべき内容と改善案を具体的に教えてください。

[ここにエクスポートしたMarkdownを貼り付け]
```

**個別フィードバック生成（生徒向け）**
```
以下は○○単元の小テスト結果データです。
各生徒（学籍番号）ごとに、間違えた問題の傾向と学習アドバイスを
1〜2文でまとめてください。
...
- [ ] `lib/db/quizzes.ts` に `getUnitQuizResultsForExport(unitId, grade, classNum)` を追加
  - 単元内の全レッスン × 全設問 × 全生徒（対象クラス）の回答・正誤を取得
  - 学年・クラス絞り込みは Phase 13 / 15.9 と同じ `student_number` の桁構造を利用
  - 記述式は回答テキストを含める（`quiz_attempt_answers.answer`）
- [ ] `GET /api/teacher/units/[unitId]/quiz-export?grade=x&class=x&format=markdown|csv` を実装
  - Markdown または CSV 形式で返す（`Content-Disposition: attachment` でダウンロード）
  - teacher/admin のみアクセス可
- [ ] `/teacher/contents` の単元行にエクスポートボタンを追加
  - クリックでモーダルを開き、学年・クラス・形式（Markdown / CSV）を選択してダウンロード
  - モーダル内に AI 分析用プロンプトテンプレートをコピーできる UI を設置
  - 小テストが存在しない単元ではボタンを非表示にする

### 活用イメージ

1. 単元終了後、教師がエクスポートボタンから Markdown をダウンロード
2. Claude 等に「つまずきポイント分析プロンプト」とセットで貼り付けて分析を依頼
3. 分析結果をもとに次回授業のスライド・発問を修正
4. 個別フィードバックが必要な生徒は `/teacher/students` の備考欄にメモを記入

### マージ判断

teacher ロール以外がエクスポートできないこと、出力データが学籍番号での匿名化にとどまり氏名・メールアドレス等の個人情報を含まないことを確認してからマージ
